...
import { validateFields, validateAllowedFields } from './validate.js';
...
function getRecipient(fields, options, log) {
  const [toStr, toJson] = toJsonOrString(options.to);
  const to = toStr || toJson[fields[options.tokenField]];
  if (!to) {
    log.error('No email address found in the form', { toStr, toJson }, `Add a field named ${options.tokenField} with a value that matches one of the tokens in the TO env var`);
    throw new Error(`No email address found for token: ${fields[options.tokenField]}`);
  }
  return to
}

function getHookParsed(fields, hookJson, options, log) {
  ...
  if (!key) {
    log.warn(`No token found in the form for field ${options.tokenField}`);
    return null;
  }
  return hookJson[key];
}

function getHook(fields, options, log) {
  if(!options.hook) {
    return null;
  }
  ...
  try {
    if (typeof options.hook === 'string') {
      ...
      return getHookParsed(fields, hookJson, options, log);
    } else {
      return getHookParsed(fields, options.hook, options, log);
    }
  } catch(e) {
    log.error('hook option is not a JSON object', e.message, options.hook);
...
export async function handleForm(fields, refererHeader, options, headersString) {
  const log = createLogger(fields, options);
  log.log('Processing form with fields:', fields);

  const { valid: allowedValid, errors: allowedErrors } = validateAllowedFields(fields, options);
  if (!allowedValid) {
    log.warn('Rejected form — disallowed fields:', allowedErrors);
    return;
  }

  const referer = fields[options.siteField] || refererHeader || 'your website';
  const hook = getHook(fields, options, log);
  const html = generateEmailBody(fields, referer, options);
  try {
    const to = getRecipient(fields, options, log);
    if (fields[options.honeyField]) {
      if (hook) {
        await sendHook(html, to, headersString, hook, fields, false, 'Honey pot field was filled');
      }
      log.log('Honeypot field filled. Not sending email.');
      return;
    }
    ...
    log.info('Email sent successfully to', to, 'Response:', result?.response);
  } catch (err) {
    if (hook) {
